...
def read_Dst(infile, debug = False):
    '''
     This function read an DST file and parses it into a special data structure.
     
     Usage:
         >>>data = read_Dst('some_file_name.dat')
        The returned value is a dictionary of values read from the file.
        
        The *debug* kwarg, if set to **True**, will create a debug print outs
    ...
    data['Date']=np.zeros(nLines, dtype='object') # a vector of time objects!
    data['Mean_Dst']=np.zeros(nLines)
    data['Hour']= list(range(1,24))
    data['Dst_hrly']= np.array([[0 for x in range(24)] for x in range(nLines)])
    parts = ['', '','','']
    ...
    for i, l in enumerate(lines):
        ...
        year = parts[1][1:3]+temp[3:5]
        month = temp[5:7]
        date = temp[8:10]
       
        t = (year, month, date)
        tNow = ' '.join(t) 
        data['Date'][i]=dt.datetime.strptime(tNow, '%Y %m %d')
        for j in range(24):
            data['Dst_hrly'][i][j]= parts[2][j*4:j*4+4]
        data['Mean_Dst'][i]= parts[3]  
